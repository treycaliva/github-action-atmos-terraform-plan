...
This Github Action is used to run Terraform plan for a single, Atmos-supported component and save the given planfile to S3, Azure Blob Storage, or Google Cloud Storage.
...
The action supports AWS, Azure, and GCP to store Terraform plan files. 
  ...
  github:
    gitops:
      ...
      artifact-storage:
        ...
        google-project-id: my-gcp-project-123
        google-firestore-database-name: "(default)"
        google-firestore-collection-name: terraform-plan-storage
      # GCP authentication should be configured externally using google-github-actions/auth
      matrix:
        sort-by: .stack_slug
        group-by: .stack_slug | split("-") | [.[0], .[2]] | join("-")
```
...
> [!NOTE]
> GCP authentication is handled externally. Configure `google-github-actions/auth@v2` with
> Workload Identity Federation in your workflow before calling this action.
      ...
      steps:
        - name: Authenticate to Google Cloud
          ...
          with:
            ...
            service_account: 'terraform-plan@my-project.iam.gserviceaccount.com'

        - name: Plan Atmos Component
          uses: cloudposse/github-action-atmos-terraform-plan@v6
          with:
            component: "foobar"
            stack: "plat-ue2-sandbox"
            atmos-config-path: ./rootfs/usr/local/etc/atmos/
...
- `v6` adds GCP (Google Cloud Storage + Firestore) plan and metadata storage support
- `v6` upgrades `cloudposse/github-action-terraform-plan-storage` from `v1` to `v2`
- GCP authentication is handled externally via `google-github-actions/auth@v2`

No breaking changes for existing AWS or Azure users.
